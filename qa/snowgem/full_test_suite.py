#!/usr/bin/env python2
#
# Execute all of the automated tests related to Commercium.
#
...
def test_rpath_runpath(filename):
    output = subprocess.check_output(
        [repofile('qa/commercium/checksec.sh'), '--file', repofile(filename)]
    )
...
def test_fortify_source(filename):
    proc = subprocess.Popen(
        [repofile('qa/commercium/checksec.sh'), '--fortify-file', repofile(filename)],
...
def check_security_hardening():
    ...
    ret &= test_rpath_runpath('src/commerciumd')
    ret &= test_rpath_runpath('src/commercium-cli')
    ret &= test_rpath_runpath('src/commercium-gtest')
    ret &= test_rpath_runpath('src/commercium-tx')
    ret &= test_rpath_runpath('src/test/test_bitcoin')
    ret &= test_rpath_runpath('src/commercium/GenerateParams')

    # NOTE: checksec.sh does not reliably determine whether FORTIFY_SOURCE
    # is enabled for the entire binary. See issue #915.
    ret &= test_fortify_source('src/commerciumd')
    ret &= test_fortify_source('src/commercium-cli')
    ret &= test_fortify_source('src/commercium-gtest')
    ret &= test_fortify_source('src/commercium-tx')
    ret &= test_fortify_source('src/test/test_bitcoin')
    ret &= test_fortify_source('src/commercium/GenerateParams')
...
STAGE_COMMANDS = {
    ...
    'gtest': [repofile('src/commercium-gtest')],
