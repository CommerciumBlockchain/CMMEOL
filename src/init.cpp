...
ZCJoinSplit* pcommerciumParams = NULL;
...
void Shutdown()
{
    ...
    RenameThread("commercium-shutoff");
    ...
    pwalletMain = NULL;
#endif
    delete pcommerciumParams;
    pcommerciumParams = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "commercium.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "commerciumd.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 2019, 12020));
    ...
    strUsage += HelpMessageOpt("-enablecommerciumsend=<n>",strprintf(_("Enable use of automated darksend for funds stored in this wallet (0-1, default: %u)"), 0));
    strUsage += HelpMessageOpt("-commerciumsendrounds=<n>",strprintf(_("Use N separate masternodes to anonymize funds  (2-8, default: %u)"), 2));
    strUsage += HelpMessageOpt("-anonymizecommerciumamount=<n>",strprintf(_("Keep N COMMERCIUM anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Commercium specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 12020, 22020));
...
{
    RenameThread("commercium-loadblk");
    // -reindex
    if (fReindex) {
        CImportingNow imp;
        int nFile = 0;
...
)
{
    ...
    if (!(
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("Cannot find the Commercium network parameters in the following directory:\n"
              "%s\n"
              "Please run 'commercium-fetch-params' or './zcutil/fetch-params.sh' and then restart."),
                ZC_GetParamsDir()),
    ...
    pcommerciumParams = ZCJoinSplit::Prepared(vk_path.string(), pk_path.string());
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Commercium version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Commercium is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Commercium is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Commercium is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    // Initialize Commercium circuit parameters
    ...
    while (!fLoaded) {
        ...
        do {
            try {
				...
				// Commercium: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Commercium") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Commercium to complete") << "\n";
    ...
    if (GetBoolArg("-minetolocalwallet", false)) {
        return InitError(_("Commercium was not built with wallet support. Set -minetolocalwallet=0 to use -mineraddress, or rebuild Commercium with wallet support."));
    }
    if (GetArg("-mineraddress", "").empty() && GetBoolArg("-gen", false)) {
        return InitError(_("Commercium was not built with wallet support. Set -mineraddress, or rebuild Commercium with wallet support."));
    }
 #endif // !ENABLE_WALLET
    ...
    fEnableCommerciumSend = GetBoolArg("-enablecommerciumsend", false);

    nCommerciumSendRounds = GetArg("-commerciumsendrounds", 10);
    if (nCommerciumSendRounds > 100) nCommerciumSendRounds = 100;
    if (nCommerciumSendRounds < 10) nCommerciumSendRounds = 10;

    nLiquidityProvider = GetArg("-liquidityprovider", 0); //0-100
    if(nLiquidityProvider != 0) {
        obfuScationPool.SetMinBlockSpacing(std::min(nLiquidityProvider,100)*15);
        fEnableCommerciumSend = true;
        nCommerciumSendRounds = 99999;
    }

    nAnonymizeCommerciumAmount = GetArg("-anonymizecommerciumamount", 0);
    if (nAnonymizeCommerciumAmount > 999999) nAnonymizeCommerciumAmount = 999999;
    if (nAnonymizeCommerciumAmount < 2) nAnonymizeCommerciumAmount = 2;
    ...
    LogPrintf("Commercium send rounds %d\n", nCommerciumSendRounds);
    LogPrintf("Anonymize Commercium Amount %d\n", nAnonymizeCommerciumAmount);
    /* Denominations
