...
const string strMessageMagic = "Commercium Signed Message:\n";
                      ...
                      libzcash::ProofVerifier& verifier)
{
    ...
    if (!CheckTransactionWithoutProofVerification(tx, state)) {
        ...
        BOOST_FOREACH(const JSDescription &joinsplit, tx.vjoinsplit) {
            if (!joinsplit.Verify(*pcommerciumParams, verifier, tx.joinSplitPubKey)) {
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    // In Commercium this has been enforced since launch, except that the genesis
    // block didn't include the height in the coinbase (see Commercium protocol spec
