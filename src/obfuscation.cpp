    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Commercium and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 CMM per transaction on average.
            */
            if (r <= 10) {
...
{
    if (!fEnableCommerciumSend && !fMasterNode) return;
...
bool CObfuscationPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    if (!fEnableCommerciumSend) return false;
    if (fMasterNode) return false;
    if (state == POOL_STATUS_ERROR || state == POOL_STATUS_SUCCESS) return false;
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeCommerciumAmount * COIN - pwalletMain->GetAnonymizedBalance();
    ...
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nCommerciumSendRounds)) {
        nValueIn = 0;
        vCoins.clear();
    ...
    if (!sessionFoundMasternode) {
        ...
        if (nUseQueue > 33) {
            ...
            BOOST_FOREACH (CObfuscationQueue& dsq, vecObfuscationQueue) {
                ...
                if (!pwalletMain->SelectCoinsByDenominations(dsq.nDenom, nValueMin, nBalanceNeedsAnonymized, vTempCoins, vTempCoins2, nValueIn, 0, nCommerciumSendRounds)) {
...
bool CObfuscationPool::PrepareObfuscationDenominate()
{
    ...
    for (int i = 0; i < nCommerciumSendRounds; i++) {
    ...
    strError = pwalletMain->PrepareObfuscationDenominate(0, nCommerciumSendRounds);
...
bool CObfuscationPool::MakeCollateralAmounts()
{
    ...
    bool success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
        nFeeRet, nChangePos, strFail, &coinControl, ONLY_NONDENOMINATED_NOT100000IFMN);
    if (!success) {
        // if we failed (most likeky not enough funds), try to use all coins instead -
        // MN-like funds should not be touched in any case and we can't mix denominated without collaterals anyway
        nChangePos = -1;
        CCoinControl* coinControlNull = NULL;
        LogPrintf("MakeCollateralAmounts: ONLY_NONDENOMINATED_NOT100000IFMN Error - %s\n", strFail);
        success = pwalletMain->CreateTransaction(vecSend, wtx, reservekeyChange,
            nFeeRet, nChangePos, strFail, coinControlNull, ONLY_NOT100000IFMN);
        if (!success) {
            LogPrintf("MakeCollateralAmounts: ONLY_NOT100000IFMN Error - %s\n", strFail);
...
void ThreadCheckObfuScationPool()
{
    ...
    RenameThread("commercium-obfuscation");
