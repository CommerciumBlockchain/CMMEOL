...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!IsValidDestination(dest)) {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Commercium address: ") + name_);
        }
...
UniValue walletpassphrase(const UniValue& params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending Commercium\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue& params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            + HelpExampleCli("signmessage", "\"commerciumaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; Commercium server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    if (params.size() > 2) {
        ...
        for (size_t idx = 0; idx < inputs.size(); idx++) {
            ...
            if (!IsValidDestination(dest)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Commercium address: ") + input.get_str());
            }
...
UniValue zc_raw_joinsplit(const UniValue& params, bool fHelp)
{
...

    {
        ...
        assert(jsdesc.Verify(*pcommerciumParams, verifier, joinSplitPubKey));
    }
    ...
    std::string encryptedNote2;
    {
        ...
        ss2 << jsdesc.h_sig(*pcommerciumParams, joinSplitPubKey);
