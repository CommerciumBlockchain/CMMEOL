                ...
                if (!item.second.nullifier) {
                    if (GetNoteDecryptor(item.second.address, dec)) {
                        ...
                        auto hSig = wtxItem.second.vjoinsplit[i].h_sig(
                            *pcommerciumParams, wtxItem.second.joinSplitPubKey);
...
mapSproutNoteData_t CWallet::FindMySproutNotes(const CTransaction &tx) const
{
    ...
    for (size_t i = 0; i < tx.vjoinsplit.size(); i++) {
        auto hSig = tx.vjoinsplit[i].h_sig(*pcommerciumParams, tx.joinSplitPubKey);
...
int CWallet::GetInputObfuscationRounds(CTxIn in) const
{
    ...
    return realObfuscationRounds > nCommerciumSendRounds ? nCommerciumSendRounds : realObfuscationRounds;
}
...
CAmount CWalletTx::GetAnonymizedCredit(bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        if (rounds >= nCommerciumSendRounds) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            if (!MoneyRange(nCredit))
...
CAmount CWalletTx::GetAnonymizableCredit(bool fUseCache) const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++) {
        ...
        const int rounds = pwallet->GetInputObfuscationRounds(vin);
        if (rounds >= -2 && rounds < nCommerciumSendRounds) {
            nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
            if (!MoneyRange(nCredit))
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it) {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputObfuscationRounds(vin);
                nTotal += pcoin->vout[i].nValue * rounds / nCommerciumSendRounds;
            }
        }
    }
    ...
    if (coinControl && coinControl->HasSelected() && !coinControl->fAllowOtherInputs)
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if (coin_type == ONLY_DENOMINATED) {
                ...
                if (rounds < nCommerciumSendRounds) continue;
            }
    ...
    bool ignoreLocked)
{
    ...
    for (auto & p : mapWallet) {
        ...
        for (auto & pair : wtx.mapSproutNoteData) {
            ...
            auto hSig = wtx.vjoinsplit[i].h_sig(*pcommerciumParams, wtx.joinSplitPubKey);
            try {
