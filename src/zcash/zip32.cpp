...
const unsigned char COMMERCIUM_HD_SEED_FP_PERSONAL[crypto_generichash_blake2b_PERSONALBYTES] =
    {'Z', 'c', 'a', 's', 'h', '_', 'H', 'D', '_', 'S', 'e', 'e', 'd', '_', 'F', 'P'};

const unsigned char COMMERCIUM_TADDR_OVK_PERSONAL[crypto_generichash_blake2b_PERSONALBYTES] =
...
{
    CBLAKE2bWriter h(SER_GETHASH, 0, COMMERCIUM_HD_SEED_FP_PERSONAL);
    h << seed;
    return h.GetHash();
}
...
uint256 ovkForShieldingFromTaddr(HDSeed& seed) {
    ...
    assert(crypto_generichash_blake2b_init_salt_personal(
        ...
        64,
        NULL,    // No salt.
        COMMERCIUM_TADDR_OVK_PERSONAL) == 0);
